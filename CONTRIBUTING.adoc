...
+
It also defines the `bt-pytest` shell function which wraps `pytest` to
use the configured Python interpreter and adds an option to set the
pytest cache directory to be within the build directory.
...
Path fixtures::
    ...
    `build_tests_dir`, `data_dir`, and `ctf_traces_dir`.
...
`run_cli()`::
    Runs the built `babeltrace2-for-tests` CLI with custom
