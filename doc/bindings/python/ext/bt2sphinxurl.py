...
def _bt2man_role(
    ...
    if not m:
        msg = f"Cannot parse manual page reference `{text}`"
    ...
    vol_node = docutils.nodes.inline(rawtext, f"({vol})")
    ...
    if page.startswith("babeltrace2"):
        ...
        # link node
        url = f"https://babeltrace.org/docs/v{bt2_version}/man{vol}/{page}.{vol}/"
...
def _bt2link_role(
    ...
    if not m:
        msg = f"Cannot parse link template `{text}`"
        inliner.reporter.severe(msg, line=lineno)
        return [inliner.problematic(rawtext, rawtext, msg)], [msg]
